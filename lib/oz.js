...
Oz.prototype._render = function (template, ctx, scope, ignoreCache) {
  ...
  findWithSelf(template, selector).filter(filterRoot(tagKeys, template)).forEach(function (el) {
    ...
    tagKeys.forEach(function (key) {
      ...
      // the function should return either null or an object
      // The object can have a `.scope` and a `.ctx`, defining
      // change to the scope and context for this element's children
      // Tags CAN overwrite each other, so you shouldn't use two tags
      // that change scope on the same element.
      ret = tags[key].render.call(self, el, ctx, prop, scope);

      if(ret) {
        // we use Object.keys here instead of just asking for ret.scope
        // so we can tell the difference between { scope: undefined } and {}
        Object.keys(ret).forEach(function (p) {
          if(p === 'scope') {
            _scope = ret[p];
          } else if(p === 'ctx') {
            _ctx = ret[p];
